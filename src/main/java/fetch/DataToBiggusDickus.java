 ...
 * @author vince
 */
public class DataToBiggusDickus {
    
    public DataToBiggusDickus(){ }
    
        public String getSessionKey(String outboundDate, String cabinClass, String destinationPlace, int adults) throws UnirestException {
        HttpResponse<JsonNode> response = Unirest.post("https://skyscanner-skyscanner-flight-search-v1.p.rapidapi.com/apiservices/pricing/v1.0")
                ...
                .field("currency", "DKK")
                .field("locale", "da-DK")
                .field("originPlace", "CPH-sky") // ("originPlace", "SFO-sky")
    ...
    public List<FlightInfoDTO> getFlightData(String outboundDate, String cabinClass, String destinationPlace, int adults) throws UnirestException {
        String sessionkey = getSessionKey(outboundDate, cabinClass, destinationPlace, adults);
...
//                System.out.println("-----AGENTS: " + agents);
        ...
        for (int i = 0; i < listLegs.length(); i++) {
            ...
            for (int j = 0; j < listItinerariesDTO.size(); j++) {
                if (listItinerariesDTO.get(j).getOutboundLegId().equals(id)) {
                            FlightInfoDTO flightInfoDTO = new FlightInfoDTO(id, "CPH-sky", destinationPlace, departure, arrival, duration, listItinerariesDTO.get(j).getPrice(), listItinerariesDTO.get(j).getDeeplinkUrl());
    ...
    public static void main(String[] args) throws InterruptedException, ExecutionException, UnirestException {
        DataToBiggusDickus swap = new DataToBiggusDickus();
        System.out.println(swap.getFlightData("2019-12-01", "economy", "LHR-sky", 1));
